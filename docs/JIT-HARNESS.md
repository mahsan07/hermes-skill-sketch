...
This package expresses **Sketch** as a typed, task-adaptive agent harness. Its concrete capability remains: Create throwaway HTML mockups with two or three design variants for comparison. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in Sketch |
| --- | --- | --- |
| Memory | `artifact_revision_history` | Retain product question plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `iterative_composition_plan` | Transform shared content model into the ordered, bounded stages defined by this skill. |
...
| Action | `brief_generate_render_inspect_export` | Advance the workflow through variant a while preserving stop conditions and user authority. |
